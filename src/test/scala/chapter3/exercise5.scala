...
class Ch3Ex5 extends FlatSpec {
  ...
  "dropWhile" should "return list if predicate returns false" in {
    assert(Exercise5.dropWhile(List(1,2,3), (x: Int) => false) == List(1,2,3))
  }

  "dropWhile" should "return Nil if predicate always returns true" in {
    assert(Exercise5.dropWhile(List(1,2,3), (x: Int) => true) == Nil)
  }

  "dropWhile" should "return Nil if list is nil" in {
